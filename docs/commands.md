...
| `$wcbet` | World Cup match betting with virtual coins. Posts a public button; clicking opens **your own** ephemeral betting panel (Components V2 stepper): opt in once for 10,000 coins, pick one of today's group-stage matches (until kickoff), choose home/draw/away, stake coins. Correct bets pay 1.5x the stake (floored); one editable bet per match; +500 daily allowance claimed lazily. **Currently gated for testing** — the public command is disabled; the owner-only `$wcbettest` runs the identical flow. See [`cogs/wcbet.md`](./cogs/wcbet.md). |
