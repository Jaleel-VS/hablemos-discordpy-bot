import { useCallback, useEffect, useState } from "react";
import {
  startPhrasal,
  submitPhrasal,
  type PhrasalView,
  type StartOptions,
} from "../../api";
import type { GameProps } from "../registry";
import Setup from "./Setup";
import Learn from "./Learn";
import Exercise from "./Exercise";
import Summary from "./Summary";

// The phrasal game has two branches off the setup screen:
//   "learn"    — a read-only vocabulary browser (no game state, no scoring).
//   exercise   — the graded fill-in-the-blank round (setup → playing → done).
type Screen = "setup" | "learn" | "playing" | "done";

export default function Phrasal({ accessToken }: GameProps) {
  const [screen, setScreen] = useState<Screen>("setup");
  const [sealed, setSealed] = useState<string | null>(null);
  const [view, setView] = useState<PhrasalView | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);

  const begin = useCallback(
    async (mode: "daily" | "free", options?: StartOptions) => {
      setBusy(true);
      setError(null);
      try {
        const resp = await startPhrasal(accessToken, mode, options);
        setSealed(resp.sealed_state);
        setView(resp.view);
        setScreen(resp.view.status === "over" ? "done" : "playing");
      } catch (e) {
        setError(e instanceof Error ? e.message : "No se pudo iniciar");
      } finally {
        setBusy(false);
      }
    },
    [accessToken],
  );

  const answer = useCallback(
    async (guess: string, finish = false) => {
      if (!sealed || busy) return;
      setBusy(true);
      setError(null);
      try {
        const resp = await submitPhrasal(accessToken, sealed, guess, finish);
        setSealed(resp.sealed_state);
        setView(resp.view);
        if (resp.view.status === "over") setScreen("done");
      } catch (e) {
        setError(e instanceof Error ? e.message : "Error al enviar");
      } finally {
        setBusy(false);
      }
    },
    [accessToken, sealed, busy],
  );

  const finish = useCallback(() => {
    void answer("", true);
  }, [answer]);

  // Auto-dismiss the round error toast so it doesn't linger.
  useEffect(() => {
    if (!error || screen !== "playing") return;
    const id = window.setTimeout(() => setError(null), 1800);
    return () => window.clearTimeout(id);
  }, [error, screen]);

  if (screen === "setup") {
    return (
      <Setup
        onStart={begin}
        onLearn={() => setScreen("learn")}
        busy={busy}
        error={error}
      />
    );
  }

  if (screen === "learn") {
    return <Learn onBack={() => setScreen("setup")} />;
  }

  if (screen === "done" && view?.result) {
    return <Summary result={view.result} onReplay={() => setScreen("setup")} />;
  }

  if (view) {
    return (
      <Exercise
        view={view}
        busy={busy}
        error={error}
        onAnswer={answer}
        onFinish={finish}
      />
    );
  }

  return (
    <div className="phrasal">
      <p className="muted">Cargando…</p>
    </div>
  );
}
