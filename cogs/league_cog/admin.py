...
class LeagueAdminCog(BaseCog):
    ...
    async def _handle_endround(self, ctx):
        ...
        Delegates to LeagueCog.process_round_end() for all round-end logic
        (saving winners, role management, announcement, next round creation).
        """
        try:
            ...
            embed.add_field(
                name="New Round",
                value=f"**Round {result['next_round_number']}** has started!",
                inline=False
            )
            embed.add_field(
                name="Ends",
            ...
            embed.add_field(
                name="Winners Saved",
                value=f"Spanish: {len(result['spanish_top3'])} | English: {len(result['english_top3'])}",
                inline=False
            )
            embed.add_field(
                name="Champion Role",
                value=f"Added: {len(result['roles_added'])} | Removed: {len(result['roles_removed'])}",
                inline=False
            )
